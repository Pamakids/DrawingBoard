...
*****************************************************/
	 ...
	 * 		a. 同时仅可播放一个绘制过程!!
	 */
final public class DrawingBoard implements IProcess
{
	...
	public function set saving( b:Boolean ) : void
	{
		...
		if (m_saving)
		{
			ProcessManager.addFrameProcess(this)
			if (!m_bytes)
			{
	...
	public function drawLine( localX:Number, localY:Number, prevLocalX:Number, prevLocalY:Number, scale:Number = 1, prevScale:Number = 1, brushIndex:int = 0 ) : void
	{
		...
		for (i = 1; i <= l; i++)
		{
			...
			m_matrix.identity()
			m_matrix.translate(-brush.m_brushPivotX, -brush.m_brushPivotY)
			m_matrix.scale(prevScale + extraScale * i / l, prevScale + extraScale * i / l)
			m_matrix.rotate(Math.random() * Math.PI * 2.0)
			m_matrix.translate(prevLocalX + extraX * i / l, prevLocalY + extraY * i / l)
			m_board.draw(brush.m_data, m_matrix, m_colorTransform, m_blendMode, null, true)
		}
		m_board.unlock()
	...
	{
		var localX:Number, localY:Number, prevLocalX:Number, prevLocalY:Number, scale:Number, prevScale:Number, time:Number, completeTime:Number = 0
