...
public class VertiLayout extends LayoutBase {
	...
	public function VertiLayout( gapY:Number, gapX:Number = -1, maxLength:int = -1, startX:Number = 0, startY:Number = 0,endX:Number = 0, endY:Number = 0 ) {
	...
	override public function activate( item:Fusion, index:int ) : void {
		if (m_maxLength <= 0) {
			item.x = m_startX
			item.y = m_startY + m_gapY * index
		}
		else {
			item.x = m_startX + m_gapX * int(index / m_maxLength)
			item.y = m_startY + m_gapY * (index % m_maxLength)
		}
