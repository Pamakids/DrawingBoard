...
//				}
			...
			{ //if the tweens are plugins, like bezier or bezierThrough for example, we cannot assume the angle between the current _x/_y and the destination ones is what it should start at!
		...
		{
			if (_changeFactor != 1)
			{ //if the tween is on a TimelineLite/Max that eventually completes, another tween might have affected the target's alpha in which case we don't want to mess with it - only disable() if it's mid-tween.
				disable();
			}
		}
		...
		override public function set changeFactor(n:Number):void
		{
			...
			{ //setting a tolerance of 2 pixels helps eliminate floating point error funkiness.
			...
			if (_blur.blurX < 1 || _target.parent == null || n == 0)
			{ //when the strength/blur is less than zero can cause the appearance of vibration. Also, if the _target was removed from the stage, we should remove the Bitmap too
			...
			{ //in case it's too big and would exceed the 2880 maximum in Flash
