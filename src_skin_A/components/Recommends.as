...
//			trace(TouchManager.getInstance().numTouchs)
		...
		private function dataLoadedHandler(datas:String):void
		{
			...
			addEventListener(RecommendEvent.RECOMMENDEVENT, onClickRecommend);
		...
		protected function onClickRecommend(e:RecommendEvent):void
		{
			crtUrl=e.url;
			if (!hint)
			{
				...
				maskS.graphics.drawRect(0, 0, height, width);
...
//				dispatchEvent(new Event(Recommends.DEACTIVE));
		...
		private function initExpandedContents():void
		{
			...
			{
				dispatchEvent(new RecommendEvent(recommendsData.main.url));
