...
//			}
			...
			UserBehaviorAnalysis.trackView('画画');
		}
		...
		override public function exit():void
		{
			...
			UserBehaviorAnalysis.trackTime('C', drawingTime, '画画');
		}
...
//			if(mPaper.drawPoint(point.x, point.y)){
		...
		// 用以记录绘画的总时间
		private var recordTime:int;
		private var drawingTime:int;
...
//			mPaper.drawLine(pointA.x, pointA.y,pointB.x, pointB.y)
		...
		private function __onRelease(e:AEvent):void
		{
			...
			drawingTime+=getTimer() - recordTime;
		}
...
//		}
		...
		private function onRandomCreatePaster(e:DataEvent):void
		{
			...
			TweenLite.from(ges, 0.4, {x: data[0], y: data[1], scaleX: Config.PASTER_LIST_ITEM_SCALE, scaleY: Config.PASTER_LIST_ITEM_SCALE, onComplete: function():void
			{
				ges.userData=true
			}})
		}
...
//			ges.y = y
		...
		private var gesIndexDic:Dictionary=new Dictionary();
		...
		{
			UserBehaviorAnalysis.trackEvent('C', '024', gesIndexDic[ges] + '', int(gesIndexDic[ges]));
			delete gesIndexDic[ges];
